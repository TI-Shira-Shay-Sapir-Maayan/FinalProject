...
def UT_HAR_dataset(root_dir):
    data_list = glob.glob(root_dir + '/move/*.csv') + glob.glob(root_dir + '/withoutMove/*.csv')
...
class CSI_Dataset(Dataset):
    ...
    def __init__(self, root_dir, modal='CSIamp', transform=None, few_shot=False, k=5, single_trace=True):
        ...
        self.data_list = glob.glob(root_dir + '/MOVE/*.csv') + glob.glob(root_dir + '/WITHOUTMOVE/*.csv')
        self.folder = glob.glob(root_dir+'/*/')
        self.category = {'move': 0, 'withoutMove': 1}  # דוגמה אם יש לך רק את שתי התוויות הללו
