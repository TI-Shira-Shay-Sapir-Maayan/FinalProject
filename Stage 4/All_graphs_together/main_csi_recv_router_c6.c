...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info)
{
    ...
    const wifi_pkt_rx_ctrl_t *rx_seq = &info->rx_seq;
...
printf("CSI_DATA- " MACSTR ", rx_seq: %d, s_count: %d, rssi: %d, rate: %d, timestamp: %d, sig_len: %d, rx_state: %d, channel: %d, cur_bb_format: %d, cur_single_mpdu: %d, noise_floor: %d, rxend_state: %d, second: %d, rx_channel_estimate_info_vld: %d, rx_channel_estimate_len: %d, he_siga1: %lu, he_siga2: %d, he_sigb_len: %d, is_group: %d, rxmatch0: %d, rxmatch1: %d, rxmatch2: %d, rxmatch3: %d", 
    MAC2STR(info->mac), 
    info->rx_seq, s_count++, rx_ctrl->rssi, rx_ctrl->rate, rx_ctrl->timestamp, rx_ctrl->sig_len,
    rx_ctrl->rx_state, rx_ctrl->channel, rx_ctrl->cur_bb_format,
    ...
    //     int real = info->buf[i];
    //     int imag = info->buf[i + 1];

    //     float amplitude = sqrt(real * real + imag * imag);
    //     float phase = atan2(imag, real);

    //     printf("Subcarrier %d: Amplitude = %f, Phase = %f\n", i / 2, amplitude, phase);
    // }
    }
